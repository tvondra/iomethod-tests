...
create table results_freebsd_d16 (seq int, cnt int, dataset text, scan text, prefetch text, eic int, version int, io_method text, io_workers int, shared_buffers text, random_page_cost text, start_val int, end_val int, optimal text, total_rows int, rows int, total_pages int, pages int, cost numeric, timing_cold numeric, timing_warm numeric);
EOF

sqlite3 scans.db <<EOF
.mode csv
.separator ' '
.import results.csv results_freebsd_d16
EOF

sqlite3 scans.db <<EOF
create index idx1 on results_freebsd_d16 (dataset,scan,prefetch,eic,io_method,io_workers,shared_buffers,random_page_cost);
analyze;
EOF
